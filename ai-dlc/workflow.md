...
**CRITICAL**: When performing any phase, you MUST read and use relevant content from rule detail files. These files are bundled inside THIS SKILL's `rules/` directory (the same skill folder that contains this `workflow.md`).

All subsequent rule detail file references in this document (e.g., `common/process-overview.md`, `inception/workspace-detection.md`) are relative to that `rules/` directory — read them as `<skill-dir>/rules/common/process-overview.md`, `<skill-dir>/rules/inception/workspace-detection.md`, and so on, where `<skill-dir>` is the directory this `workflow.md` lives in.
