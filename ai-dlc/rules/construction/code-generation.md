...
- [ ] Create explicit steps for unit generation:
  ...
  - Frontend Components Generation (if applicable — see Wireframe Seeding Rules when `requirements/screens/` exists)
...
### Wireframe Seeding Rules (prd-flow integration)
When `requirements/screens/` exists (wireframe `*.jsx` + `*_description.md` ingested from a prd-flow working directory):
- **Seed, don't regenerate**: use the wireframe JSX as the starting point for frontend components — preserve its screen structure, component decomposition, and layout instead of inventing a new UI
- **Wire real logic**: replace wireframe mock data (`MOCK_*`) with real API/state integration per the unit design
- **Honor the descriptions**: each `*_description.md` documents data binding, formulas, conditional display, validation, and state transitions per numbered component — implement them and map each `desc` prop location to a stable `data-testid` following the naming rules above (then remove the `desc` props from production code)
- **Design system**: if `prd-flow/{slug}/design/design-system.md` exists, its §0 Hard Constraints and token values govern all frontend styling
