...
#### Step 4.1: Screen Specifications Intake (prd-flow integration)

**IF `requirements/screens/` exists** (populated by SKILL.md STEP 0 from a prd-flow working directory, containing wireframe `*.jsx` files and `*_description.md` screen specification documents):
- Read every `*_description.md` — these document per-component data binding, formulas, conditional display, interactions, state transitions, and validation that the wireframe cannot show
- Treat the JSX wireframes + descriptions as CONFIRMED UI/UX requirements: do NOT re-ask clarifying questions about screen layout, flows, or interaction details already covered there
- In Step 5, mark the User Scenarios and UI-related Functional Requirements areas as covered by these artifacts (cite the source file), and focus questions on the remaining gaps (non-functional requirements, technical context, data/backend concerns)
- Reference the screen artifacts in `requirements.md` (Step 7) so downstream stages (User Stories, Application Design, Code Generation) know to consume them
