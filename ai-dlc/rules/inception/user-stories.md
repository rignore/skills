...
## Step 0: Reuse Upstream Personas and Screen Specs (prd-flow integration)

**IF the PRD was ingested from a prd-flow working directory** (SKILL.md STEP 0 path 0):
- Load `prd-flow/{slug}/gate1/03-personas.md` and REUSE those personas as the story personas — do not invent a parallel persona set. Extend only if a persona needed for a story is missing, and note the addition.
- Use `requirements/screens/` (`*.jsx` wireframes + `*_description.md`) as the user-journey source: screens map to journeys, and description documents supply acceptance-criteria details (validation, state transitions, edge cases).
- Skip clarifying questions (Step 3) for areas these artifacts already answer; cite the source file instead.
