...
`inspection-mode`(desc 편집)·`prd-to-wireframe`(화면 코드) 등으로 프로토타입(디스크립션)을 **직접 편집**해 **제품 범위(화면 추가·제외·하드코딩 결정)나 정책이 바뀐 경우**, 그 편집은 prd-sync를 자동 트리거하지 않는다. 그러나 이는 **PRD가 SoT인 `EP-`의 범위 변경**이므로, 편집을 마친 직후 prd-sync를 호출해 PRD 정합을 맞춰야 drift를 막는다.

- 화면을 빼거나, 단일 화면으로 합치거나, 임계·정책을 하드코딩으로 고정한 결정이 Epic의 포함 범위나 완료 상태를 바꾸면 PRD `EP-` 범위 변경이다. **역전파 컨펌**으로 PRD를 갱신한다.
- 수식·표시 형식 같은 디스크립션 SoT 내부 변경은 PRD로 역전파하지 않는다(레이어 분리 유지). 단 PRD `EP-` 서술에 구식 표현이 남아 있으면 그 표현만 정리한다.
...
| 접두 | 대상 | 예 | SoT 문서 |
|------|------|----|---------|
| `EP-{n}` | Epic (독립적인 사용자 가치·운영 역량·완료 결과 단위) | `EP-01` (알림 운영 상태 확보) | PRD Epic 정의 |
...
기존 프로젝트의 `FN-{n}`은 이전 기능 정의를 읽고 동기화할 때만 유지한다. 신규 PRD에 `FN-`을 부여하거나 `EP-`와 `FN-`을 같은 문서의 동급 정본으로 혼용하지 않는다.

### 앵커 병기 의무

의존 문서는 SoT 정보를 참조할 때 **참조한 앵커 ID를 반드시 병기**한다.
- 예: 디스크립션 임계 판정 기준 옆에 `(앵커 D-4, 근거 PRD Decision Log)`
- 예: 와이어프레임 manifest의 화면 항목 옆에 `(앵커 EP-01)`
...
| Epic 정의 (`EP-`) | PRD (완료 결과 단위, 우선순위 별도 섹션) | 디스크립션·와이어프레임(범위 근거) |
